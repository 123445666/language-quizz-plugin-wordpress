$(function () {
        ...
        getDataJson().then(function (data) {
            ...
            if (quiz.isEnded()) {
                ...
                for (var i = 0; i < choices.length; i++) {
                    ...
                    element.innerHTML = choices[i].image ? '<img class="object-cover rounded-lg shadow border-2 border-solid border-white transition-all ease-in-out duration-300 cursor-pointer" src="' + choices[i].image + '"/>' : choices[i].name;
    ...
    function showScores() {
        ...
        gameOverHTML += "<h3 id='score' class='flex-auto text-5xl mb-5'> Your scores: <br/><div class='text-red-300 text-center'>" + localStorage.getItem("currentQuestionNumber") + "</div></h3>";
        gameOverHTML += "<div id='quizz-reset' class='mt-3 text-center sm:mt-0 py-2 px-8 bg-red-300 hover:bg-red-200 font-bold text-white rounded-lg shadow-md text-sm cursor-pointer'> Bắt đầu lại Quiz </div>";
        gameOverHTML += "</div>";
